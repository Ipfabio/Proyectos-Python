...
# Snake
# Tupla, formando el cuerpo empezando por la cabeza
...
while True:
    ...
    if time_since_start >= 0.5:
        ...
        # Check Death
        # Para asegurar que estemos dentro del mapa y no nos comamos
        if not 0 <= snake_body[0][0] <= CELL_NUM - 1 or not 0 <= snake_body[0][1] <= CELL_NUM - 1 or snake_body[0] in snake_body[1:]:
            break

        # "Limpia" lo que deja la serpiente al moverse
        field.DrawRectangle((0, 0), (FIELD_SIZE, FIELD_SIZE), 'black')

        tl, br = convert_pos_to_pixel(apple_pos)
        field.DrawRectangle(tl, br, 'red')  # topleft, bottomright, color

        # draw Snake
        for index, part in enumerate(snake_body):
            tl, br = convert_pos_to_pixel(part)
            color = 'yellow' if index == 0 else 'green'  # Ponemos la cabeza en amarillo
            field.DrawRectangle(tl, br, color)

window.close()
# TODO Al morir deje algún mensaje, aumentarle la velocidad y la capacidad de elegir dificultades